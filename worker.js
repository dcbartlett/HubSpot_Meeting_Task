...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    const contactAssociationsResults = (await 
      hubspotClient.crm.associations.batchApi.read('MEETINGS','CONTACTS',{ inputs: meetingsToAssociate.map(meetingId => ({ id: meetingId })) })
    )?.results || [];

    const contactAssociations = Object.fromEntries(contactAssociationsResults.map(a => {
      if (a._from) {
        meetingsToAssociate.splice(meetingsToAssociate.indexOf(a._from.id), 1);
        return [a._from.id, a.to[0].id];
      } else return false;
    }).filter(x => x));

    const promises = data.map(meeting => {
      return (async ()=> {
        if (
          !meeting.properties ||
          !meeting.properties.title ||
          !meeting.properties.timestamp
        ) return;
        ...
        const actionTemplate = {
          ...
          meetingProperties: {
            ...
            meeting_attendeeId: contact.properties.email
          }
        };
